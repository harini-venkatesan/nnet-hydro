...
#         if isinstance(random_field, torch.Tensor) and random_field.any():
#             self.conductivity.vector()[:] = \
#                 torch.exp(self.field_mean +
#                           self.field_stdev * random_field).numpy()

#         else:
#             shape = self.mesh.coordinates().shape[0]
#             self.conductivity.vector()[:] = \
#                 torch.exp(self.field_mean *
#                           torch.ones(shape)).numpy()
...
#     def get_data(self, datapoints):
#         self.data = torch.zeros(len(datapoints))
#         for i, datapoint in enumerate(datapoints):
#             self.data[i] = self.h(datapoint[0],
#                                   datapoint[1])
#         return self.data
